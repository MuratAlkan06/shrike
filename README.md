...
**What that leaves.** Idle, read, and write are each a number now, so a connection cannot be held by silence, by half a request, or by a peer that stops listening — what one connection can cost this broker is `max.request.bytes` of heap and at most those bounds of time, and what every connection can cost is `connectionCap` times that. Time is bounded in both directions; the memory half of the envelope is `connectionCap` × `max.request.bytes` — 64 × 4 MiB by default, and at most 1024 × 1 GiB, because **both of those settings now have a largest value as well as a smallest one and a configuration past either is refused at startup**. That is a number rather than a hope at the maxima as well as at the defaults, and it is not a small one: the ceilings say what this broker can be asked for, not what it recommends. What none of these bounds is, is access control: this build has no authentication, which is a [non-goal](#non-goals) rather than an open question, and the loopback default is what stands in for it.
