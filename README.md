...
**One broker at a time owns a data directory.** A start takes an exclusive lock on `shrike.lock` inside the directory before it opens a single log, so a second broker pointed at the same one refuses to start and names the directory rather than writing into it beside the first — the partition logs, the topic registry, and the committed offsets in there all assume one writer. The lock is the operating system's rather than a pid written to a file, so it goes when the process goes however it goes: a broker that was killed leaves a lock file that holds nothing, the next start takes it as it stands, and there is nothing to delete by hand.
...
**The broker's shipped entry point reads twenty environment variables and nothing else.** `BrokerMain` builds a `BrokerLaunch` out of the environment — no configuration file, no command line, and an argument is refused rather than silently ignored — and hands the broker the `BrokerConfig` those variables describe. The container image runs that same entry point, so it reads the same twenty. **One rule names all of them: a setting's variable is `SHRIKE_` followed by the setting's own name in capitals, with each dot an underscore** — `retention.ms` is `SHRIKE_RETENTION_MS`, `flush.interval.bytes` is `SHRIKE_FLUSH_INTERVAL_BYTES`, and the three settings that have no dotted name are spelled the same way from their field names.
...
| `SHRIKE_MAX_FETCH_WAIT_MS` | `30000` | the longest a fetch may be held open, whatever wait it asks for; at most `3600000`, an hour |
| `SHRIKE_MAX_REQUEST_BYTES` | `4194304` (4 MiB) | the largest request frame this broker will read, and so the most one connection can make it hold; at most `1073741824` (1 GiB), the largest record this broker can store |
...
| `write.timeout.ms` | `SHRIKE_WRITE_TIMEOUT_MS` | `writeTimeoutMs` | `BrokerConfig` |
...
**How long a connection may spend reading, how long it may sit idle, and how long an answer to it may make no progress are each a number.** A connection may spend at most `read.timeout.ms` — 30 seconds by default, from `SHRIKE_READ_TIMEOUT_MS` — in one *read phase*, and a read phase is exactly two things: the time it sits idle between requests, and the time it spends part way through a request frame. A thread named `shrike-conn-reaper` asks once per bound and closes the connections that have crossed it, which is what brings their threads out of their blocking reads and gives their places under the connection cap back. **The bound is elapsed time, not the host's date.** It is measured by subtracting two readings of a clock that only climbs, so a wall clock stepped an hour forwards by a time-synchronization daemon does not close every connection at once, and one stepped an hour backwards does not stretch the bound by an hour. `SO_TIMEOUT` is not what does it: that bounds reads on a socket's streams and not on the `SocketChannel` the broker reads through. **The bound never covers serving a request.** A long-poll fetch held open for `max.fetch.wait.ms` is the broker waiting to write rather than a client failing to speak, and it is not closed for it — a fetch waiting the full 30 seconds under a read timeout of one second is served its records, which is what `BrokerReadTimeoutTest#keepsALongPollFetchThatWaitsFarPastTheReadTimeoutAndServesItItsRecords` is there to keep true. **What the read bound does not cover is the write side, and a second number does.** So what `read.timeout.ms` covers is reading and idling, and the sentence stops there. There is no value that turns it off, and the largest one it takes is a ceiling rather than an off switch: it is a whole number of milliseconds that must be at least 1, and an hour — `3600000` of them — is as long as one read phase can be allowed to run.

**A write that is getting nowhere is bounded; a write that is merely slow is not.** `write.timeout.ms` — 30 seconds by default, from `SHRIKE_WRITE_TIMEOUT_MS` — bounds the time an answer this broker is sending spends with no byte of it leaving. **It is not a cap on how long an answer takes.** Every time bytes actually go, the bound starts again, so a peer draining a fetch of several mebibytes slowly but steadily is never closed however many bounds the whole answer costs, which is what `BrokerWriteTimeoutTest#servesAnAnswerLargerThanTheSocketBuffersToAPeerThatDrainsItSlowly` keeps true; and a peer that asks for an answer larger than the socket buffers and then stops reading is closed one bound after it stops, which is `BrokerWriteTimeoutTest#closesAPeerThatStoppedReadingItsAnswerAndGivesItsPlaceUnderTheCapBack`. Closing the socket is what brings that connection's thread out of the write it is parked in, and its place under the connection cap goes back on that thread, through the one path that gives a place back however a connection ends. **What counts as bytes leaving is a report from the write loops, and neither loop asks for more than a mebibyte at a time**: a blocking write, and a blocking transfer out of a segment file, do not return until everything they were asked for has gone, so a single call for a whole answer would report nothing until the answer had arrived. The bound is therefore crossed by a peer that has not taken a mebibyte of its answer in `write.timeout.ms` — about 35 KiB a second at the default — and that number is the resolution of this bound rather than a rate limit. **A long-poll fetch is not writing yet**, so nothing here reaches it: the bound begins with the first byte of an answer, and `max.fetch.wait.ms` is what bounds the wait before it. Like the read bound it is measured on elapsed time, so a stepped wall clock does not move it in either direction, and like the read bound it must be at least 1 millisecond and at most an hour of them.

**What that leaves.** Idle, read, and write are each a number now, so a connection cannot be held by silence, by half a request, or by a peer that stops listening. **What none of the three is, is a bound on how long a connection may live.** Each bounds a *phase*: one read phase, one answer with no byte of it moving. A peer that sends one small request per bound, or that takes one mebibyte of its answer per bound — about 35 KiB a second at the default — is making progress by both definitions and is never closed, so it can hold its place under the connection cap for as long as it keeps paying that much. That is what a progress bound means rather than an oversight, and the number it leaves is worth writing down: one answer can be held for `ceil(answerBytes ÷ 1 MiB) × write.timeout.ms`, which is two minutes for a 4 MiB fetch at the defaults and, at the largest values this broker accepts, a gibibyte of records at an hour per mebibyte — about 43 days, or twice that once the reaper's overshoot is counted. A zero-copy fetch also holds one segment descriptor open for as long as that lasts, so a deleted segment's bytes stay on disk until the peer is done with them. What bounds a peer that keeps paying is the connection cap, not the clock. The memory half of the envelope is `connectionCap` × `max.request.bytes` — 64 × 4 MiB by default, and at most 1024 × 1 GiB, because **both of those settings now have a largest value as well as a smallest one and a configuration past either is refused at startup**. That product is exact while `fetch.zero.copy` is on, which is the default; with it off, a fetch's records are read into an array and then copied into the response frame, so read it as about twice the product. That is a number rather than a hope at the maxima as well as at the defaults, and it is not a small one: the ceilings say what this broker can be asked for, not what it recommends. What none of these bounds is, is access control: this build has no authentication, which is a [non-goal](#non-goals) rather than an open question, and the loopback default is what stands in for it.
...
This slice puts a number on three things an adjective was carrying: what a whole produce costs under each `flush.mode` rather than only what one record costs, how much of the buffered fetch path's cost is the mebibyte it allocates per fetch, and how many times a long poll opens its segment file however often it is woken. All of them are JMH benchmarks under `shrike-core/src/test/java/io/shrike/core/bench/`, compiled by every build and run by none of it, and the `bench` profile carries one execution per run:

```
mvn -pl shrike-core -P bench test-compile exec:exec@suite
mvn -pl shrike-core -P bench test-compile exec:exec@per-record-tail
mvn -pl shrike-core -P bench test-compile exec:exec@per-record-long-trial
mvn -pl shrike-core -P bench test-compile exec:exec@fetch-allocation
mvn -pl shrike-core -P bench test-compile exec:exec@fetch-allocation-small-heap
```
...
Its heap is that JVM's own default on this machine, which is G1 with a 12 GiB maximum and 8 MiB regions. That is stated here because one paragraph below turns on it.

**Why a placeholder and not the commit the runs were started from.** Slice branches are squash-merged into `main` in this repository, so the branch commit a benchmark process was launched at does not exist in this history under its own identity and a fresh clone cannot resolve it. Naming it would leave a reference that dangles while looking like evidence, and naming the squash is only possible once the squash exists. The branch commit these runs were launched from is recorded in the pull request that carried them, so the replacement can be checked rather than trusted.

**Five runs produced everything below**, each writing a result file of its own, and each file records its own forks, iteration counts, parameters, and JVM arguments:

| Execution | What it ran | Committed result |
|---|---|---|
| `suite` | every benchmark, one thread, 2 forks, 3 × 1 s warmup, 5 × 1 s measurement, no JVM arguments | [`v1.6-flush-and-fetch.json`](docs/bench/v1.6-flush-and-fetch.json) |
| `per-record-tail` | the sampled `per-record` arm again, 2 forks, 3 × 1 s warmup, 30 × 2 s measurement | [`v1.6-per-record-tail.json`](docs/bench/v1.6-per-record-tail.json) |
| `per-record-long-trial` | one `per-record` throughput arm, 4 forks, 3 × 1 s warmup, 20 × 3 s measurement | [`v1.6-per-record-long-trial.json`](docs/bench/v1.6-per-record-long-trial.json) |
| `fetch-allocation` | the fetch paths under JMH's `gc` profiler, at the suite's iterations | [`v1.6-fetch-allocation.json`](docs/bench/v1.6-fetch-allocation.json) |
| `fetch-allocation-small-heap` | the same under `-Xmx512m`, which that file records as the JVM argument it was | [`v1.6-fetch-allocation-small-heap.json`](docs/bench/v1.6-fetch-allocation-small-heap.json) |

Each is committed as JMH wrote it, and every figure below is read out of one of them.

**What a flush mode costs a produce.** `FlushPolicyBenchmark` appends the records of one produce — a 162-byte frame each, a 128-byte value and no key — to a log opened with the defaults apart from the mode, one record at a time, which is what `Partition.produce` does with a produce request's records under one lock. One invocation is one produce and `batchRecords` says how many records it carries: one is the per-record figure, and a hundred is a batch a client library would send without anybody thinking it large. The ± is JMH's own 99.9% confidence interval.

| `flush.mode` | Records per produce | Produces per second | p50, closed-loop service time | p99, closed-loop service time | Samples timed |
|---|---|---|---|---|---|
| `per-record` | 1 | 247.2 ± 3.4 | 4.00 ms | 7.48 ms | 28 958 |
| `per-record` | 100 | 2.44 ± 0.13 | 402.65 ms | not published | 300 |
| `interval`, 100 ms / 1 MiB | 1 | 487 451 ± 59 113 | 0.96 µs | 3.79 µs | 333 071 |
| `interval`, 100 ms / 1 MiB | 100 | 5 290.1 ± 343.3 | 108.93 µs | 4.27 ms | 53 304 |

The rate column and both `interval` percentile rows come from the `suite` file. The two `per-record` percentile rows come from the `per-record-tail` file, which is the same benchmark over thirty two-second measured iterations in each of two forks instead of five one-second ones — a hundred and twenty measured seconds rather than ten, for the reason the next paragraph gives.

**A p99 is worth what its sample count is worth, and the `per-record` count used to be thin.** At about four milliseconds a force, the suite's ten measured seconds can only time some 2 456 `per-record` produces, which puts about twenty-five samples above the ninety-ninth percentile. The deeper run times **28 958**, close to twelve times as many and about three hundred above the percentile, and it moved the answer: the p99 read off the deep run is 7.48 ms against 6.34 ms off the shallow one and 5.97 ms off the Slice 5 run ([`docs/bench/slice-5-flush-and-fetch.json`](docs/bench/slice-5-flush-and-fetch.json)). That direction is the useful part. A longer run does not make a tail heavier; it finds more of the rare events that were always there, so the earlier figure was thin *and* optimistic, and the number in the table is the one measured over more of them. Beyond it, p99.9 is 15.03 ms and the slowest of the 28 958 was 173.80 ms.

The 100-record `per-record` row is timed 300 times over the same deep run, which is three samples above its ninety-ninth percentile, so no p99 is published for it: its median and its rate are what that row can honestly say.

**Per-record is per record, and a produce pays for every one of them.** A single-record produce forces once and takes 4.00 ms at its median; a hundred-record produce forces a hundred times and takes 402.65 ms, which is 100.6 medians. The rates say it the other way round: 2.44 produces a second at a hundred records is 244 appends a second, against the 247.2 the one-record row sustains. A client that batches to amortize the network amortizes nothing against the device here: under this mode what a produce costs is how many records are in it.

**Both percentile columns are closed-loop service time, and that phrase is load-bearing.** The harness issues the next produce only once the previous one has returned, so nothing ever queues behind anything: these are the times the log took, not the times a client would have waited under an arrival rate the broker does not control. A percentile measured this way understates latency under open load, which is why it is written into the column heading rather than left to be assumed.

**The `per-record` rate is not bimodal, and one long trial is what says so.** In the Slice 5 run ([`docs/bench/slice-5-flush-and-fetch.json`](docs/bench/slice-5-flush-and-fetch.json)) the two forks of that arm disagreed — one sustained 245 to 249 produces a second and the other 181 to 213 — and the rate was published as 222.1 ± 41.5 with a paragraph naming the split. The `per-record-long-trial` execution exists to answer whether that was the mode or the machine: four forks, twenty three-second measured iterations each, eighty measured iterations in all, 241.07 ± 3.25 produces a second overall. Two of the four forks ran their first eleven measured iterations between 220.6 and 233.9 and every iteration after that between 240.9 and 248.9; the other two never dropped below 241.4 at any point. Every one of the four reached 248.8 or better at its fastest. A mode that was bimodal would not converge: it would keep both rates for the whole of a four-minute trial and it would do it in every fork. What this shows instead is a slow start that clears after about thirty seconds of forcing, in some JVMs and not others — machine and device state, which is also what the Slice 5 write-up guessed and could not then show.

**A sample is not a produce.** JMH's `SampleTime` mode times some invocations rather than all of them, and it thins that subset as a run goes on, so the `Samples timed` column counts the produces that were timed and not the produces that were made. The 333 071 on the one-record `interval` row were drawn from several million; the 28 958 on the one-record `per-record` row are very nearly every produce that run made. Each percentile stands on the count printed beside it.

**Only the `interval` rows roll a segment.** At the rates in the table a `per-record` trial appends a few thousand records over its warmup and measured seconds — some hundreds of kibibytes of frames — and never comes near the 128 MiB `segment.bytes`, so it rolls nothing at all. An `interval` trial appends millions in the same seconds, hundreds of mebibytes, and rolls and seals a segment about every 128 MiB while it does. The roll, and the force that seals a segment, are charged to the `interval` rows alone, and they are what puts a millisecond p99 on a row whose median is a tenth of one. That tail is a property of a trial of this length rather than a constant of the mode: a shorter trial meets fewer seals and a longer one meets more.

**What serving a fetch out of the file costs.** `FetchPathBenchmark` serves one fetch of a fixed 1 MiB range — 991 frames, 1 048 478 bytes — of a pre-built log into a connected pair of loopback `SocketChannel`s, with a thread reading the other end and discarding it. The two rows are the two calls `fetch.zero.copy` selects between and nothing else.

| Path | Fetches per second | Bytes per second (derived) |
|---|---|---|
| `FileChannel.transferTo` out of the segment file | 2 484.0 ± 38.6 | 2 484 MiB/s |
| read into a buffer, then one `writeFully` | 2 149.4 ± 77.6 | 2 149 MiB/s |

The `Bytes per second` column is derived rather than measured: it is the row beside it multiplied by the 1 048 478-byte range, and it carries that row's ± with it. Both rows include loopback TCP and the thread draining it, which is deliberate — a transfer differs from a read-then-write only when the destination is a socket, so a sink that was not one would have compared the buffered path with itself. Neither row is a measurement of a disk, of a network, or of any machine other than this one.

**The floor under each row is measured rather than modelled, and the two floors are not the same number.** `FetchPathBenchmark.transferWarmFileToTheSinkAlone` transfers the same bytes out of an already-warm file into the same loopback pair with no log behind it — no range located, no descriptor opened — and it ran at 26 090 ± 737 transfers a second, which is 38.3 µs to put the mebibyte on the socket that way. `FetchPathBenchmark.writeRangeToTheSinkAlone` writes the same bytes out of a heap array into the same pair with no log behind it, and it ran at 14 591 ± 510 writes a second, which is 68.5 µs. They differ because a file-to-socket transfer and a heap-to-socket write are different work, and that is the correction this slice owes: the Slice 5 write-up priced one shared sink and took the same 68.3 µs off both rows, which charged the transfer row for a copy it never makes.

Read against their own floors, the transfer row spends 38.3 µs of its 402.6 on the socket and the buffered row 68.5 µs of its 465.2, so neither is bound by it and both pay it. What is left — the log's own share of a fetch — is 364.3 µs against 396.7 µs, so the buffered path costs **8.9%** more once each row has paid its own sink. Subtracting the buffered floor from both, as the earlier write-up did, would have said 18.8%: the model overstated the difference between the two paths by about a factor of two, in the direction that flattered the newer path. The table itself is unaffected — the transfer row served 15.6% more fetches a second than the buffered row, as measured — and both floors are in the same committed file as the two paths, deliberately kept out of the table beside them because neither is a path a fetch can take.

**Most of what is left is a mebibyte the buffered path allocates per fetch.** `SegmentedLog.readRange` answers with a `byte[]` sized to the range, so every buffered fetch allocates a fresh 1 048 478-byte array; the transfer path hands the file's own bytes to the kernel. Under JMH's `gc` profiler the buffered path allocated **1 143 930 bytes per fetch** at 2 363 MB/s against the transfer path's **95 785 bytes**, and the difference — 1 048 145 bytes — is within a few hundred bytes of the range itself. So the gap between the two rows is copy plus allocation, and the allocation is the range buffer rather than anything subtler. The transfer path is not allocation-free either, and the number beside it says so.

**What that allocation costs the collector depends on the heap, and both heaps were measured.** At the default 12 GiB heap G1's regions are 8 MiB, so a 1 MiB array is a large allocation but not a humongous one: the profiled trial collected 53 times and spent 23 ms doing it. At `-Xmx512m` G1's regions are 1 MiB, the same array is humongous, and the same trial collected 97 times and spent 41 ms — nearly twice the collections for the same bytes. It did not show up in the score: the buffered path ran at 2 169.5 ± 49.2 fetches a second on the default heap and 2 157.9 ± 48.0 on the small one, which is inside either interval. So the honest reading is that the buffered path's cost here is the copy and the zeroing rather than the collector's response to it, at least down to a 512 MiB heap on this machine — which is the opposite of what a paragraph of reasoning about humongous allocation would have predicted, and the reason the arm was run instead of reasoned about. The scores in this paragraph are the profiled run's own, and a profiler is not free; the unprofiled rates are the ones in the table above.
...
**A long poll opens that descriptor once, however many times it is woken.** `LongPollOpenCountBenchmark` drives one fetch through a stated number of wakeups before it can be answered, and counts — beside the score, as JMH aux counters that land in the same result file — the segment files it opened, the times it registered as a waiter, and the fetches served. A fetch that finds too few bytes holds the range it located and asks again through that descriptor, so the `open(2)` the first wakeup with records pays is the only one there is.

| Wakeups before the fetch could be answered | Long polls served | Segment files opened | Opens per fetch |
|---|---|---|---|
| 1 | 299 994 | 299 994 | 1 |
| 8 | 49 510 | 49 510 | 1 |
| 64 | 4 630 | 4 630 | 1 |

Opens equalled fetches to the unit at every wakeup count, so the sixty-four-wakeup row is one `open(2)` where a fetch that let go of its range each time it waited would have made sixty-four. **That is a count and not a latency claim.** The rate JMH prints beside those counters is two threads passing a partition lock back and forth as fast as this harness can drive them, which is a property of the harness rather than of a broker, and it is deliberately not published.

**Interval-mode append throughput is where it was.** The one-record `interval` row sustains 487 451 ± 59 113 produces a second, each of them one append, against 485 739 ± 76 685 for the same benchmark in the Slice 5 run ([`docs/bench/slice-5-flush-and-fetch.json`](docs/bench/slice-5-flush-and-fetch.json)) — 0.35% apart, with each figure well inside the other's interval. Those are different machine-days rather than a controlled comparison, so what this supports is the absence of a regression rather than the presence of an improvement: making a log's unforced-byte count a volatile field, so a flush sweep can pass over a clean partition without taking its lock, did not cost the append path anything this benchmark can see.
...
| On the machine, the JVM, and the harness commit named under Benchmarks, one produce of a 162-byte frame measured 247.2 ± 3.4 produces a second under `per-record` — one force — and 2.44 ± 0.13 for a 100-record produce of a hundred forces, under macOS `fsync(2)` and not `F_FULLFSYNC`, against 487 451 ± 59 113 and 5 290.1 ± 343.3 under `interval` | `docs/bench/v1.6-flush-and-fetch.json` | 1.6 |
| On that same machine, commit, and JVM, the closed-loop p99 service time of one one-record produce measured 7.48 ms under `per-record` over 28 958 timed samples of a hundred and twenty measured seconds, with a p99.9 of 15.03 ms | `docs/bench/v1.6-per-record-tail.json` | 1.6 |
| On that same machine, commit, and JVM, the closed-loop p99 service time of one one-record produce measured 3.79 µs under `interval` over 333 071 timed samples drawn from several million produces, and 4.27 ms for a 100-record produce over 53 304 | `docs/bench/v1.6-flush-and-fetch.json` | 1.6 |
| On that same machine, commit, and JVM, the `per-record` rate held together across four forks of twenty three-second iterations at 241.07 ± 3.25 produces a second, every fork reaching 248.8 or better and the two that started slow leaving that state after eleven iterations, which is what says the Slice 5 fork split was machine state rather than the mode | `docs/bench/v1.6-per-record-long-trial.json` | 1.6 |
| On that same machine, commit, and JVM, serving one 1 MiB range into a loopback socket measured 2 484.0 ± 38.6 fetches a second through `FileChannel.transferTo` and 2 149.4 ± 77.6 through a buffered read | `docs/bench/v1.6-flush-and-fetch.json` | 1.6 |
| On that same machine, commit, and JVM, the floor under each fetch path — the same bytes into the same loopback socket with no log behind them — measured 26 090 ± 737 transfers a second out of a warm file and 14 591 ± 510 writes a second out of a heap array, so each row costs 38.3 µs and 68.5 µs of its own for the socket rather than one shared figure | `docs/bench/v1.6-flush-and-fetch.json` | 1.6 |
| On that same machine, commit, and JVM, under JMH's `gc` profiler the buffered fetch path allocated 1 143 930 bytes per fetch at 2 363 MB/s against the transfer path's 95 785, a difference of 1 048 145 bytes for a 1 048 478-byte range | `docs/bench/v1.6-fetch-allocation.json` | 1.6 |
| On that same machine, commit, and JVM, shrinking the heap to `-Xmx512m` — where G1's regions are 1 MiB and that array is humongous — took the buffered path's collections from 53 to 97 and its collection time from 23 ms to 41 ms, while its rate stayed inside its interval at 2 157.9 ± 48.0 against 2 169.5 ± 49.2 | `docs/bench/v1.6-fetch-allocation-small-heap.json` | 1.6 |
| On that same machine, commit, and JVM, a long poll opened its segment file once per fetch whatever the wakeups: opens equalled long polls served at 1, 8, and 64 wakeups — 299 994, 49 510, and 4 630 of each | `docs/bench/v1.6-flush-and-fetch.json` | 1.6 |
| On that same machine, commit, and JVM, one-record `interval` produces measured 487 451 ± 59 113 a second against the 485 739 ± 76 685 the Slice 5 run measured for the same benchmark, 0.35% apart and each inside the other's interval | `docs/bench/slice-5-flush-and-fetch.json` | 1.6 |
...
| The read bound is measured on elapsed time, so stepping the wall clock an hour in either direction neither closes a connection nor postpones one, and the connection is closed by elapsed time alone | `BrokerReadTimeoutTest#measuresTheBoundOnElapsedTimeSoAWallClockStepDoesNotMoveIt` | 1.2 |
| The clock those deadlines are measured on starts at zero and never hands out a reading smaller than one it has already handed out | `SystemMonotonicTimeSourceTest#handsOutNoReadingSmallerThanOneItHasAlreadyHandedOut` | 1.2 |
...
| A peer draining an answer larger than the socket buffers slowly but steadily is served every byte of it, though the transfer costs many times `write.timeout.ms` in total: what is bounded is making no progress, not how long an answer takes | `BrokerWriteTimeoutTest#servesAnAnswerLargerThanTheSocketBuffersToAPeerThatDrainsItSlowly` | 1.2 |
| A peer that asks for an answer larger than the socket buffers and then stops reading it is closed one bound after it stops, and the place it held under the connection cap comes back, so the next connection is served | `BrokerWriteTimeoutTest#closesAPeerThatStoppedReadingItsAnswerAndGivesItsPlaceUnderTheCapBack` | 1.2 |
| A connection whose answer has already gone is never closed by the write bound, however many bounds pass before its next request | `BrokerWriteTimeoutTest#neverClosesAConnectionWhoseAnswerHasAlreadyGone` | 1.2 |
| A long-poll fetch that has not begun writing is untouched by the write bound, however many of them go by while it waits, and is answered with its records | `BrokerWriteTimeoutTest#keepsALongPollThatHasNotBegunWritingHoweverManyWriteBoundsGoBy` | 1.2 |
| The write bound is measured on elapsed time too, so stepping the wall clock an hour in either direction neither closes a connection being written to nor postpones one | `BrokerWriteTimeoutTest#measuresTheWriteBoundOnElapsedTimeSoAWallClockStepDoesNotMoveIt` | 1.2 |
| The write loop reports what left each time some of it does, and reports nothing for a call that moved no bytes at all | `ByteChannelsTest#saysNothingForACallThatMovedNoBytesAtAll` | 1.2 |
| A transfer of a range out of a segment file reports what has gone as it goes, and what it reports adds up to the range it promised | `SegmentedLogRangeTest#tellsWriteProgressWhatOfTheRangeHasGoneAsItGoes` | 1.2 |
| A `write.timeout.ms` of zero stops the start naming the variable that set it, rather than starting a broker that closes every connection it answers | `BrokerLaunchTest#refusesAWriteTimeoutOfZeroNamingTheVariableThatWouldHaveClosedEveryAnswer` | 1.2 |
...
| A group whose first commit fails before its file is moved into the groups directory is forgotten along with the place it took under the budget, so the groups after it still commit | `GroupOffsetStoreTest#forgetsAGroupWhoseCreatingCommitFailedBeforeItsFileWasMovedIntoPlace` | 1.1 |
| A group whose first commit fails after its file is moved into place keeps the group and its place under the budget, because the file is there for the next start to load — and the commit is still refused, so the store counts exactly the group files the directory holds | `GroupOffsetStoreTest#keepsAGroupWhoseCreatingCommitMovedItsFileIntoPlaceAndThenFailed` | 1.1 |
| A second broker started over a data directory a running broker holds refuses to start with a message naming that directory, and the broker holding it goes on serving | `BrokerDataDirectoryLockTest#refusesToStartOverADataDirectoryABrokerIsAlreadyRunningOverAndLeavesThatBrokerServing` | 1.2 |
| A broker that stopped releases its data directory, so the next one over it starts and serves, and the lock file it left behind is still there | `BrokerDataDirectoryLockTest#startsOverTheSameDataDirectoryOnceTheBrokerHoldingItHasStoppedCleanly` | 1.2 |
| A lock file left behind by a broker that is no longer running costs the next start nothing: it is a file rather than a claim, and nothing has to be deleted by hand | `BrokerDataDirectoryLockTest#startsOverALockFileLeftBehindByABrokerThatIsNoLongerRunning` | 1.2 |
| Refusing a second broker inside one JVM leaves the running broker's lock where it was, so a separate process asked afterwards is still told the data directory is somebody's | `BrokerDataDirectoryLockTest#keepsTheRunningBrokersLockOnTheDataDirectoryAfterRefusingASecondBrokerInThisJvm` | 1.3 |
| That holds when the second directory is a different directory whose lock file is the same file — a hard link, a bind mount: the start is refused before a descriptor on that file is opened, the lock is still the running broker's once the collector has run, and the directory refused that way is startable again once the lock is free | `BrokerDataDirectoryLockTest#keepsTheRunningBrokersLockWhenRefusingAStartOverADirectoryThatSharesItsLockFile` | 1.3 |
| A start refused by the one holder a claim cannot see — something in this JVM holding the lock file that is not a broker — keeps the descriptor that refusal opened, so the collector never closes it and drops that holder's lock | `BrokerDataDirectoryLockTest#keepsTheLockOfANonBrokerHolderInThisJvmWhenTheStartThatMetItIsRefused` | 1.3 |
| A second broker in this JVM over a data directory whose `shrike.lock` was deleted under the broker holding it is refused too, without a descriptor on the file that broker holds ever being opened — the same deletion in front of a broker in another process is not something a claim can refuse | `BrokerDataDirectoryLockTest#refusesASecondBrokerOverADirectoryWhoseLockFileWasDeletedUnderTheBrokerHoldingIt` | 1.3 |
| Where the filesystem can identify the lock file, what it keeps is one descriptor per lock file rather than one per attempt: a start retried twenty-five times over against a holder that will not let go opens nothing after the first, and the start that finally takes the lock takes it through the descriptor that was kept | `BrokerDataDirectoryLockTest#keepsOneDescriptorPerLockFileHoweverManyStartsANonBrokerHolderRefuses` | 1.3 |
| A start over a directory whose lock file the filesystem cannot identify is never handed a kept descriptor, because one kept under a directory's name can be open on a file `shrike.lock` no longer names: it opens its own and locks the file that name resolves to | `BrokerDataDirectoryLockTest#opensItsOwnDescriptorRatherThanAskingOneKeptForALockFileItCannotIdentify` | 1.3 |
| And on that path every descriptor a refusal leaves open is kept rather than one of them, since none of them will be asked for and a channel nothing holds is closed by its cleaner: one per refusal is what a filesystem with no file identity costs | `BrokerDataDirectoryLockTest#keepsEveryRefusedDescriptorWhereTheLockFileHasNoIdentityToBoundThemBy` | 1.3 |
| Every setting that decides how much memory or time one caller can make this broker hold is refused above its ceiling, naming the variable that crossed it | `BrokerLaunchTest#refusesEveryCappedSettingOneOverItsCeilingNamingTheVariableThatCrossedIt` | 1.2 |
| Each of those ceilings is itself a value this broker starts with, so what is refused begins one past it | `BrokerLaunchTest#takesEveryCappedSettingAtItsCeilingExactly` | 1.2 |
| A numeric setting whose value carries a newline is refused on one scrubbed line, so a hostile value cannot forge a second log line | `BrokerLaunchTest#refusesANumericValueWithANewlineOnOneScrubbedLineRatherThanLettingItForgeASecond` | 1.2 |
| A numeric setting set to a value too long to be a number is refused with a truncated message rather than one that floods standard error | `BrokerLaunchTest#refusesAVeryLongNumericValueWithATruncatedMessageRatherThanFloodingStandardError` | 1.2 |
| A whole number written in the digits of another script is refused naming the variable, rather than read as the number the JDK's own parse would take it for | `BrokerLaunchTest#refusesANumberWrittenInDigitsOfAnotherScriptRatherThanReadingItAsTheJdkWould` | 1.2 |
| A path variable set to a value no path can hold is refused naming the variable on one scrubbed line, rather than letting an unchecked exception escape the start carrying the raw value | `BrokerLaunchTest#refusesAPathVariableHoldingACharacterNoPathCanCarryNamingItOnOneScrubbedLine` | 1.2 |
| A symbolic link planted at the temporary file a durable write goes through is deleted rather than followed, so the file at the end of it keeps every byte and the write still lands on the target it was asked for | `DurableFileTest#deletesASymbolicLinkPlantedAtItsTemporaryFileRatherThanWritingThroughIt` | 1.3 |
| The ready file a broker announces its port in is written the same way, which is what covers the one file this build writes into a directory the broker does not own | `ReadyFileTest#deletesASymbolicLinkPlantedAtItsTemporaryFileInASharedDirectoryRatherThanWritingThroughIt` | 1.3 |
| A temporary file a crash left behind costs the next durable write nothing: it is deleted rather than written over, the write lands, and none of it is left in the directory | `DurableFileTest#deletesAStaleTemporaryFileLeftBehindByACrashRatherThanBeingBlockedByIt` | 1.3 |
...
| A fetch whose offset retention deletes while it waits is refused with the offset the partition now starts at, and the segment file it was holding is closed | `PartitionFetchDescriptorTest#closesTheSegmentFileItHeldWhenRetentionDeletesTheOffsetItIsWaitingOn` | 1.5 |
| A client that hangs up while its long poll is still waiting leaves no segment file open behind it | `BrokerFetchDescriptorTest#closesTheSegmentFileAFetchWasHoldingWhenItsClientWentAwayWhileItWaited` | 1.5 |
| An answer the write bound stopped part way through gives its segment file back as its connection unwinds | `BrokerFetchDescriptorTest#closesTheSegmentFileOfAnAnswerTheWriteBoundStoppedPartWayThrough` | 1.5 |
