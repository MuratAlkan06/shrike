...
| A start refused by the one holder a claim cannot see — something in this JVM holding the lock file that is not a broker — keeps the descriptor that refusal opened for the life of the process, so the collector never closes it and drops that holder's lock | `BrokerDataDirectoryLockTest#keepsTheLockOfANonBrokerHolderInThisJvmWhenTheStartThatMetItIsRefused` | 1.3 |
