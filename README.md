...
**Where it listens, and what it does not check.** The broker binds the loopback interface, and the bind address is not configurable. This build has no authentication, no authorization, and no transport security, so a configurable bind address belongs to the slice that makes exposing the port defensible. There is no server-side read or idle timeout either — `SO_TIMEOUT` bounds reads on a socket's streams and not on the `SocketChannel` the broker reads through — so a local connection that opens and then says nothing, or speaks very slowly, holds its slot until it is closed. What bounds that today is the loopback bind and the connection cap of 64; an idle reaper rides with the bind-address and authentication work.
