...
| Refusing a second broker inside one JVM leaves the running broker's lock where it was, so a separate process asked afterwards is still told the data directory is somebody's | `BrokerDataDirectoryLockTest#keepsTheRunningBrokersLockOnTheDataDirectoryAfterRefusingASecondBrokerInThisJvm` | 1.2 |
| That holds when the second directory is a different directory whose lock file is the same file — a hard link, a bind mount — and the directory refused that way is startable again once the lock is free | `BrokerDataDirectoryLockTest#keepsTheRunningBrokersLockWhenRefusingAStartOverADirectoryThatSharesItsLockFile` | 1.3 |
